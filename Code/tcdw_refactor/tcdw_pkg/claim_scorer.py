...
def score_single_claim_with_nli(claim: str, answer_sentences: List[str], audit_fn):
    best_entail = 0.0
    best_contra = 0.0
    best_neutral = 0.0
...
def classify_claim_status(best_entail: float, best_contra: float, entail_threshold: float = 0.50, contra_threshold: float = 0.50):
...
def score_must_have_claims(must_have_claims: List[Dict[str, Any]], generated_text: str, audit_fn, entail_threshold: float = 0.50, contra_threshold: float = 0.50):
    ...
    for item in must_have_claims:
        ...
        local_scores = score_single_claim_with_nli(claim_text, sentences, audit_fn)

        status = classify_claim_status(
            local_scores["best_entail"],
            local_scores["best_contra"],
            entail_threshold,
            contra_threshold,
        )
