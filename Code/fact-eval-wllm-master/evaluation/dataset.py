...
class ClinicDataset(BaseDataset):
    """Dataset class for HealthQA, MIMIC-3 dataset."""

    def __init__(self, data_source: str):
        """
            Initialize the HealthQA, MIMIC-3 dataset.

            Parameters:
                data_source (str): The path to the HealthQA, MIMIC-3 dataset file.
        """
        super().__init__()
        self.data_source = data_source
        self.load_data()
    ...
    def load_data(self):
        """Load data from the HealthQA, MIMIC-3 dataset file."""
        with open(self.data_source, 'r') as f:
           dataset = json.load(f)
        for item in dataset[:200]:
            self.prompts.append(item['prompt'])
            self.natural_texts.append(item['natural'])
