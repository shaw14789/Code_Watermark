...
def _parallel_run_fixed(args):
    """Helper function for parallel processing that avoids pickling issues."""
    i, encoded_text, test_result, pseudo_length, vocab_size = args
    
    # Generate random xi_alternative
    xi_alternative = np.random.rand(pseudo_length, vocab_size).astype(np.float32)
    
    # Perform one_run calculation directly (avoiding utils object)
    k = len(encoded_text)
    n = len(xi_alternative)
    A = np.empty((1, n))
    
    for j in range(n):
        A[0][j] = levenshtein(encoded_text, xi_alternative[(j + np.arange(k)) % n], 0.0)
    
    null_result = np.min(A)
...
class EXPEdit(BaseWatermark):
    ...
    def detect_watermark_parallel(self, text: str, return_dict: bool = True, num_processes: int = None, *args, **kwargs):
        """Detect watermark in the text using parallel processing for improved performance."""
        ...
        # Get the test result using the watermark's xi
        xi_numpy = self.utils.xi.cpu().numpy()
        test_result, _ = self.utils.one_run(encoded_text, xi_numpy)
        ...
        with mp.Pool(processes=num_processes) as pool:
            ...
            args_list = [(i, encoded_text, test_result, self.config.pseudo_length, self.config.vocab_size) 
                        for i in range(self.config.n_runs)]
            
            # Execute parallel runs
            results = pool.map(_parallel_run_fixed, args_list)
    ...
    def print_config(self):
        """Print the configuration of the KGW algorithm."""
        print(f"Pseudo Length: {self.config.pseudo_length} Top-K: {self.config.top_k}")
        print(f"N Runs: {self.config.n_runs} P Threshold: {self.config.p_threshold}")
        print(f"xi shape: {self.utils.xi.shape}  seq_len: {self.config.sequence_length}")
        return