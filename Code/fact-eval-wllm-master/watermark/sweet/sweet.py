...
class SWEETUtils:
    ...
    def get_greenlist_ids(self, input_ids: torch.LongTensor) -> list[int]:
        ...
        if self.config.device != "auto":
            ...
            vocab_permutation = torch.randperm(self.config.vocab_size, generator=self.rng)
...
class SWEET(BaseWatermark):
    ...
    def generate_watermarked_text(self, prompt: str, *args, **kwargs):
        ...
        # generate watermarked text
        # print("---- calling generation model ----")
    ...
    def print_config(self):
        """Print the configuration of the SWEET algorithm."""
        print(f"Gamma: {self.config.gamma} delta: {self.config.delta}  entropy_threshold: {self.config.entropy_threshold}")
        return